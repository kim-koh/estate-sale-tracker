...
function TransactionCard({transactionIndex}) {
    ...
    function deleteTransaction() {
        ...
        if (shouldDelete) {
            ...
            axios.delete(`/api/transaction/${thisTransaction._id}`, {data: {"newStockNums": newStockNums}}) // deletes item in database
                 .then(() => {
                    transactionDispatch({type: 'DELETE_TRANSACTION', payload: transactionIndex});
                    dispatch({type: 'UPDATE_STOCK', payload: newStockNums}) //updates stock client side
                 })
                 .catch(error => {
                     console.log(error)
                 })
